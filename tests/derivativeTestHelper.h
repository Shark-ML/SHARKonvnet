...
std::vector<RealVector> estimateDerivative(Model& net,const Point& point,double epsilon=1.e-10){
	...
	for(size_t parameter=0;parameter!=parameters.size();++parameter){
		...
		gradients[parameter] = result1/(2*epsilon) - result2/(2*epsilon);
	}
	return gradients;
}
template<class Model,class Point>
...
std::vector<RealVector> estimateInputDerivative(Model& net,const Point& point,double epsilon=1.e-10){
	...
	for(size_t dim=0;dim!=point.size();++dim){
		...
		gradients[dim] = result1/(2*epsilon) - result2/(2*epsilon);
	}
	return gradients;
}
...
void testWeightedDerivative(Model& net,const Point& point,const RealVector& coefficients,double epsilon=1.e-5,double estimationEpsilon = 1.e-5){
	...
	for(std::size_t coeff = 0; coeff!= coefficients.size(); ++coeff){
		...
		net.eval(pointBatch, output, *state);
		// std::cout << "pointBatch: " << pointBatch << " output: " << output << std::endl;
		...
		// std::cout << "coeffBatch: " << coeffBatch << " testGradient: " << testGradient << std::endl;
		
		//calculate error between both
		BOOST_REQUIRE_EQUAL(testGradient.size(),derivative.size());
		// std::cout << "testGradient(i)     : " << testGradient << std::endl;
		// std::cout << "coefficients(coeff) : " << coefficients(coeff) << std::endl;
		// std::cout << "coeff: " << coeff << std::endl;
		for(std::size_t i = 0; i != testGradient.size(); ++i){
			// std::cout << "derivative[i]: " << derivative[i](coeff) << std::endl;
...
void testWeightedInputDerivative(Model& net,const Point& point,const RealVector& coefficients,double epsilon=1.e-5, double estimationEpsilon=1.e-5){
	...
	RealVector testGradient0;
	// Need to eval this one first and overwrite the result:
	// net.weightedParameterDerivative(pointBatch,coeffBatch,*state,testGradient0);
	// Then we can call
	...
	// std::cout << "row(testGradient,0): " << row(testGradient,0) << std::endl;
	// std::cout << "resultGradient(i):   " << resultGradient << std::endl;
	
	double error;
	for (int i = 0; i < resultGradient.size(); i++) {
		error = row(testGradient,0)(i) - resultGradient(i);
		BOOST_CHECK_SMALL(error, epsilon);
	}
	...
	// 	std::cout << "testGradient" << std::endl << row(testGradient,0) << std::endl;
	// 	std::cout << "resultGradient" << std::endl << resultGradient << std::endl;
	// }
...
///samples are taken from the interval -10,10
template<class Model>
void testWeightedInputDerivative(Model& net,unsigned int numberOfTests = 1000, double epsilon=1.e-5,double estimationEpsilon = 1.e-5, float coeff_min = -10, float coeff_max = 10) {
	...
	for(unsigned int test = 0; test != numberOfTests; ++test){
		...
		for(size_t i = 0; i != net.outputSize();++i){
			coefficients(i) = Rng::uni(coeff_min, coeff_max);
		}
		for(size_t i = 0; i != net.inputSize();++i){
...
void testWeightedDerivativesSame(Model& net,unsigned int numberOfTests = 100, double epsilon = 1.e-10){
	...
	for(unsigned int test = 0; test != numberOfTests; ++test){
		...
		// Changed. If there are no params in the model, this failed in SHARK-impl.
		if (net.numberOfParameters() > 0) {
